...
class Hand(object):
    ...
    def update(self, word):
        ...
        for letter in self.handDict.keys():
            if word_dict.get(letter,0) != self.handDict.get(letter,0):
                 res[letter] = self.handDict.get(letter, 0) - word_dict.get(letter,0)
        self.handDict = res
    ...
    def containsLetters(self, letters):
        ...
        word_dict = getFrequencyDict(letters)
        for letter in word_dict.keys():
            if (word_dict.get(letter,0) > self.handDict.get(letter,0)) or (self.handDict.get(letter,0) == 0):
                 return False         
        return True
    ...
    def __eq__(self, other):
        ...
        """
        for letter in other.handDict.keys():
            if (other.handDict.get(letter,0) > self.handDict.get(letter,0)) or (self.handDict.get(letter,0) == 0):
                 return False         
        return True
...
class ComputerPlayer(Player):
    ...
    def pickBestWord(self, wordlist):
        ...
        top_score = 0
        res = '.'
        word_dict = wordlist.getList()
        for word in word_dict:
            freq_word = getFrequencyDict(word)
            if  self.hand.containsLetters(word):
               if  getWordScore(word) > top_score:
                   top_score = getWordScore(word)
                   res = word
        return res
